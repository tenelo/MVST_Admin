  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      if (!_isLoading && _currentPage < _totalPages) {
        _chargerUtilisateurs(page: _currentPage + 1);
      }
    }
  }

  Future<void> _chargerUtilisateurs({
    int page = 1,
          ...
          setState(() {
            if (isLoadMore) {
              _utilisateurs.addAll(
                List<Map<String, dynamic>>.from(data['utilisateurs']),
              );
            } else {
            ...
            Expanded(
              child: _isLoading && _utilisateurs.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemCount: _utilisateurs.length + (_isLoading ? 1 : 0),
                      itemBuilder: (context, index) {
                        if (index == _utilisateurs.length && _isLoading) {
                          return const Padding(
                            padding: EdgeInsets.all(16),
                            child: Center(child: CircularProgressIndicator()),
                          );
                        }
                        final compte = _utilisateurs[index];
                        final points =
